...
const getAssociatedSubjects = async () => {
	...
	form.querySelector("#subject-name").value = getDatasetValSubject("name");
	...
	selectElementSubject.addEventListener("change", (e) => {
		form.querySelector("#subject-name").value =
...
window.record.getAccounts(async (event, allAccounts) => {
